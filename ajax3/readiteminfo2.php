...
$q = sqlsrv_query($connect, @"Select Sys_Users_Goods.*,Shop_Goods.*
							from Sys_Users_Goods
							inner join Shop_Goods on Sys_Users_Goods.TemPlateID = Shop_Goods.TemplateID
							where ItemID = ?", array($id), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
...
if(sqlsrv_num_rows($q) > 0) {
	...
	if ($itemInfo['Attack'] != 0) {
		$ShowStrengthenLv = "Cường Hóa:&nbsp;&nbsp;<span class=\"span_style205\">+".$itemInfo['StrengthenLevel']."</span>";
		$ShowAttack = "Tấn Công:&nbsp;&nbsp;<span class=\"span_style205\"> " .$itemInfo['Attack']."(+".$itemInfo['AttackCompose'].")</span>";
        $ShowDefence = "Phòng thủ:&nbsp;&nbsp;<span class=\"span_style205\"> " .$itemInfo['Defence']."(+".$itemInfo['DefendCompose'].")</span>";
        $ShowAgility = "Nhanh nhẹn:&nbsp;&nbsp;<span class=\"span_style205\"> " .$itemInfo['Agility']."(+".$itemInfo['AgilityCompose'].")</span>";
        $ShowLuck = "May mắn:&nbsp;&nbsp; <span class=\"span_style205\"> " .$itemInfo['Luck']."+(".$itemInfo['LuckCompose'].")</span>";
	}
