...
if($result['type'] == -1) {
	...
	$qLoad = sqlsrv_query($connect, @"select Shop_Goods.Name, Shop_Goods.NeedSex, Shop_Goods.CategoryID, Shop_Goods.Pic,Sys_Users_Goods.ItemID, Sys_Users_Goods.TemplateID,Sys_Users_Goods.Count,Sys_Users_Goods.StrengthenLevel,Sys_Users_Goods.IsBinds,Sys_Users_Goods.BagType, Sys_Users_Goods.ValidDate 
										from Sys_Users_Goods 
										inner join  Shop_Goods on Sys_Users_Goods.TemplateID = Shop_Goods.TemplateID 	
										where Sys_Users_Goods.IsExist = 1 AND Sys_Users_Goods.UserID = ? AND Sys_Users_Goods.BagType >= 0 AND Sys_Users_Goods.BagType <= 1 ORDER by Sys_Users_Goods.Place DESC", array($characterid), array( "Scrollable" => SQLSRV_CURSOR_KEYSET));
