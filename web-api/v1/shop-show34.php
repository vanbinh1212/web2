...
if ($method == 'POST'){
	...
	if ($ShowID == null) {
		$Type = intval($Type);
		$sql  = "insert into ShopGoodsShowList(ShopID, Type) values ('$id','$Type')";
	} else {
		$ShowID = intval($ShowID);
		if ($Type == 'null' || $Type == null || !is_numeric($Type) || $Type == -999 || $Type == '-999') {
			$sql = "delete ShopGoodsShowList where TempID = '$ShowID'";
		} else {
			$Type = intval($Type);
			if ($Type != 0) {
				$sql = "update ShopGoodsShowList set [Type] = '$Type' where TempID = '$ShowID'";
			}		
		}		
	}
	$q = @$data->query($sql);
	...
	$sql = "SELECT ID,ShopID,GroupID,TemplateID,BuyType,IsContinue,
		...
		IsCheap,LimitCount,StartDate,EndDate,
		ShowID,Type,Name,NeedSex,CategoryID,Pic 
		from V_Shop id asc OFFSET (($page)*100) ROWS FETCH NEXT 100 ROWS ONLY";
    $q = @$data->query($sql);
	$array;
	$i = 0;
