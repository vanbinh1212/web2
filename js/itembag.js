...
ItemBag = {
	...
	sendItems: function() {
		...
		$.post(ItemBag.vars.urlSendItem + "?svid=" + svid + "&cid=" + cid, function(data) {
			if(data['type'] == 0) {
				SnapeClass.openModal('Thành công', "Đã gửi vật phẩm vào game thành công. Vui lòng kiểm tra thư trong game. Nếu gặp vấn đề không nhận được vật phẩm. Vui lòng liên hệ BQT Fanpage Gunny 5.5 để được hỗ trợ.", []);
				ItemBag.init();
			} else {
				SnapeClass.openModal('Lỗi lấy Vật phẩm Web', data['content'], []);
			}
