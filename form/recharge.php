...
if($return['type'] == -1) {
	if(!is_numeric($type) || $type <= 0 || strlen($serial) < 5 || strlen($passcard) < 5) {
		...
		$user = $_SESSION['ss_user_id'];
		$charge_result = $trumthe247->ChargeCard($typeCode, $serial, $passcard, $amount, $user, $serverid, $uid); //thực hiện đẩy thẻ lên hệ thống TrumThe247.Com
